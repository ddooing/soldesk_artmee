...
public interface AdminExhibitionMapper {
	...
	// ===============================================전시회 관리========================================
		...
		@Select("<script> "
				+ "SELECT e.exhibition_enroll_id,e.title, e.author, e.price, "
				+ "	TO_CHAR(e.exhibition_start, 'yyyy-mm-dd') AS exhibition_start,"
				+ "	TO_CHAR(e.exhibition_end, 'yyyy-mm-dd') AS exhibition_end,"
				+ "	e.enroll_state, u.name AS apply_name "
				+ "	FROM exhibition_enroll e "
				+ "	JOIN user_table u ON e.apply_person = u.user_id "
				+ "<if test='title != null and title != \"\"'> "
				+ " WHERE UPPER(e.title) LIKE '%' || #{title} || '%' "
				+ "</if>"
				+ "<if test='name != null and name != \"\"'> "
				+ "  WHERE UPPER(u.name) LIKE '%' || #{name} || '%'"
				+ "</if> "
				+ "<if test='author != null and author != \"\"'> "
				+ "  WHERE UPPER(e.author) LIKE '%' || #{author} || '%' "
				+ "</if> "
				+ " ORDER BY e.exhibition_enroll_id DESC"
				+ "</script>")
		List<ExhibitionDetailBean> getEnrollExhibitionSearch(@Param("title") String title,
		...
		@Select("<script> "
				...
				+ " ORDER BY e.exhibition_enroll_id DESC"
				+ "</script>")
		int getEnrollExhibitionSearchCnt(@Param("title") String title,
										@Param("name") String name,
										@Param("author") String author);	
		...
		@Select("<script>"
				...
				+ "</script>")
		ExhibitionDetailBean getEnrollExhibitionSearch_BadgeCnt(@Param("title") String title,
																@Param("name") String name,
																@Param("author") String author);		
		
		
		// 전시회 1개 상세 정보 가져가기
		@Select("SELECT e.exhibition_enroll_id,e.title,e.author,"
				+ "    e.price, TO_CHAR(e.exhibition_start, 'yyyy-mm-dd') as exhibition_start, \r\n"
				+ "    TO_CHAR(e.exhibition_end, 'yyyy-mm-dd') as exhibition_end, \r\n"
				+ "    e.open, e.holiday,e.address,e.site, e.place,"
				+ "    e.enroll_state,e.main_poster_file_id, e.detail_poster_file_id,"
				+ "	   e.apply_person,u.name AS apply_name, u.email AS apply_email, u.telephone AS apply_telephone,\r\n"
		...
		@Insert("INSERT INTO exhibition "
				+ "(exhibition_id, title, regdate, author, price, exhibition_start, "
				+ "exhibition_end, open, holiday, address, place, site, state, apply_person,"
				+ " main_poster_file_id, detail_poster_file_id) "
				+ "VALUES "
				+ "(exhibition_id_seq.NEXTVAL, #{title}, sysdate, #{author}, #{price}, TO_DATE(#{exhibition_start}, 'YYYY-MM-DD'),TO_DATE(#{exhibition_end}, 'YYYY-MM-DD'), #{place}, #{site}, #{state}, #{apply_person},#{main_poster_file_id}, #{detail_poster_file_id})")
