...
public interface AdminMapper {
	...
	@Select("SELECT\r\n"
			...
			+ "    ut.name AS user_name, ut.telephone AS telephone, "
			+ "    e.title AS title,\r\n"
			+ "    ft.name AS main_banner_name,\r\n"
			+ "    ft.path AS main_banner_path\r\n"
			+ "FROM main_banner mb\r\n"
			+ "LEFT JOIN user_table ut ON mb.apply_person_id = ut.user_id\r\n"
			+ "LEFT JOIN exhibition e ON mb.exhibition_id = e.exhibition_id\r\n"
			+ "LEFT JOIN file_table ft ON mb.banner_file_id = ft.file_id\r\n"
			+ "WHERE mb.main_banner_id = #{main_banner_id}")
	...
	@Select("SELECT \r\n"
			...
			+ "    ft.path AS sub_banner_path\r\n"
			+ "FROM sub_banner sb "
			+ "LEFT JOIN user_table ut ON sb.apply_person_id = ut.user_id\r\n"
			+ "LEFT JOIN exhibition e ON sb.exhibition_id = e.exhibition_id\r\n"
			+ "LEFT JOIN file_table ft ON sb.banner_file_id = ft.file_id\r\n"
			+ "WHERE sb.sub_banner_id = #{sub_banner_id}")
	...
	List<BannerApplyFormBean> getAllApplyMainbanner(RowBounds rowBounds);
		...
		@Select("SELECT\r\n"
				...
				+ "    ft.path AS banner_path,\r\n"
				+ "    e.title AS exhibition_title,\r\n"
				+ "    u.name AS user_name,u.telephone AS user_telephone,u.email AS user_email\r\n"
				+ "FROM banner_apply_form baf\r\n"
				+ "LEFT JOIN\r\n"
				+ "    file_table ft ON baf.banner_file_id = ft.file_id\r\n"
				+ "LEFT JOIN\r\n"
				+ "    exhibition e ON baf.exhibition_id = e.exhibition_id\r\n"
				+ "LEFT JOIN user_table u ON baf.apply_person_id = u.user_id\r\n"
				+ "WHERE baf.banner_apply_form_id = #{banner_apply_form_id}")
		BannerApplyFormBean getBannerapplyDetail(int banner_apply_form_id);
		
		// 관리자 페이지 배너 신청 받은거 관리자가 확인하고 추가 (메인배너)
		@Insert("INSERT INTO main_banner (exhibition_id, start_date, end_date, expose_order, state, regdate, pay_money, banner_file_id) VALUES (#{exhibition_id},  TO_DATE(#{start_date}, 'YYYY-MM-DD'), TO_DATE(#{end_date}, 'YYYY-MM-DD'), #{expose_order}, #{state}, sysdate, #{pay_money}, #{banner_file_id})")
		void addApplyMainBanner(MainBannerBean mainBannerBean);
		
		// 관리자 페이지 배너 신청 받은거 관리자가 확인하고 추가 (서브배너)
		@Insert("INSERT INTO sub_banner (exhibition_id, start_date, end_date, expose_order, state, regdate, pay_money, banner_file_id) VALUES (#{exhibition_id}, TO_DATE(#{start_date}, 'YYYY-MM-DD'), TO_DATE(#{end_date}, 'YYYY-MM-DD'), #{expose_order}, #{state}, sysdate, #{pay_money}, #{banner_file_id})")
