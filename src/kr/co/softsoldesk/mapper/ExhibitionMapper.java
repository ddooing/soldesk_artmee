...
public interface ExhibitionMapper {
	...
	// 전시회 상세페이지 조회
	@Select("SELECT e.exhibition_id, e.title, e.regdate, e.author, e.price,"
			+ " e.exhibition_start, e.exhibition_end, g.open, g.holiday, "
			+ "e.ticket_cnt, g.address, e.place, g.site, e.views, e.state, "
	...
	@Select("SELECT \r\n"
			...
			+ "WHERE \r\n"
			+ "    UPPER(e.title) LIKE '%' || upper(#{title}) || '%' and state = 1"
			+ "order by exhibition_id desc FETCH FIRST 4 ROWS ONLY")
	List<ExhibitionBean>allSearchExhibition(String title);
	...
	@Insert("INSERT INTO exhibition_enroll"
			+ " (title, place, regdate, author, price, enroll_state, exhibition_start, "
			+ "exhibition_end, gallery_id, main_poster_file_id, detail_poster_file_id) " +
			"VALUES (#{title}, #{place}, sysdate, #{author},#{price},  #{enroll_state}, TO_DATE(#{exhibition_start}, 'YYYY-MM-DD'),"
			+ " TO_DATE(#{exhibition_end}, 'YYYY-MM-DD'),#{gallery_id}, #{main_poster_file_id}, #{detail_poster_file_id})")
